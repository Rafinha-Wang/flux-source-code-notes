"""
本文件核心：
Kontext 高质量约束模式的终端实现, 负责运行 flux-dev-kontext 模型
是针对特定17种“黄金比例分辨率”专门训练的模型。
这五个脚本中唯一强制引入了 PixtralContentFilter (多模态大模型内容审查) 的终端, 是一个“规矩森严”的高质量产出管道
"""
import os
import re
import time
...
from flux.util import (
    aspect_ratio_to_height_width, # 比例转换工具
...
# 交互式指令解析器：比基础版多了 /ar (Aspect Ratio) 比例控制命令
...
# (省略重复的 parse_img_cond_path 注释，逻辑同上)
...
def main(
    ...
    # 此处加载了强大的 Pixtral 内容审核器 (相比常规的鉴黄模块更进一步)，防止生成不当图片
    ...
    while opts is not None:
        ...
        # 安全审查拦截机制：如果在文本提示词或输入的参考图里发现了违规内容，直接阻断生成过程
        ...
        # 核心差异：调用 prepare_kontext，它内部会严格查表 `PREFERED_KONTEXT_RESOLUTIONS` 对齐分辨率
        inp, height, width = prepare_kontext(
            t5=t5, clip=clip, prompt=opts.prompt, ae=ae, img_cond_path=opts.img_cond_path,
            target_width=opts.width, target_height=opts.height, bs=1, seed=opts.seed, device=torch_device,
        )
        
        # 开发者留下的快照：可以把第一步拼出来的含噪张量矩阵保存出来用于 debug 分析
        ...
        # 终极拦截防线：不仅查输入，也审查生成出的成品图。如果输出越界，照样扣押重来。
